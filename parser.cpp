...
ParamDecList* Parser::parseParamDecList() {
    ...
    string type;
    string id;
    if (check(Token::PD)){//Parametros sin argumentos para el main
...
FunDec* Parser::parseFunDec(){
    ...
    if(match(Token::FUN)) {
        ...
        id = previous->text;
        if(!match(Token::PI)) {
            cerr<<"Falta el token Pi"<<endl;
            exit(4);
        }
        ParamDecList* param_dec_list= parseParamDecList();
        if(!match(Token::PD)) {
            cerr<<"Falta el token PD"<<endl;
            cout<<"Curren token es: "<<current->text<<endl;
            exit(5);
        }
        Body* body = parseBody();
        // cout<<"El current token es::: "<<current->text<<endl;
        if(!match(Token::ENDFUN)) {
            cerr<<"Falta el token ENDFUN, se encontro:"<<current->text<<endl;
            exit(1);
        }
...
Stm* Parser::parseStatement() {
    ...
    if (match(Token::ID)) {
        ...
        if (check(Token::PD)) {//Caso del return vacio
            match(Token::PD);
...
Exp* Parser::parseFactor() {
    ...
    if (match(Token::NUM)) {
        ...
        if (match(Token::PI)) {//Para arglis
            FCallExp* f_call_exp = new FCallExp(id);
            // if (check(Token::PD)) {//Ningun parametro
            //     // match(Token::PD);
            //     return f_call_exp;
            // }
            ...
            if (!match(Token::PD)) {
                cerr<<"Se esperaba un pdf, pero se encontro_ "<<current->text<<endl;
                exit(9);
            }
            return  f_call_exp;
        }
