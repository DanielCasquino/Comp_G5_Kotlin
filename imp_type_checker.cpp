...
void ImpTypeChecker::visit(AssignStatement* s) {
  ...
  if (!type.match(var_type)) {
    cout << "Tipo incorrecto en Assign a " << s->id << endl << type << " : " << var_type << endl;
  }
  return;
}
...
void ImpTypeChecker::visit(ForStatement* s) {
  ...
  sp_decr(3);
  // IMPORTANT: VERIFY THE ADD_LEVEL AND REMOVE_LEVEL
  env.add_level();

  // env.add_level();
  env.add_var(s->temporalVariable, s->start->accept(this));
  //
  s->b->accept(this);
...
ImpType ImpTypeChecker::visit(BinaryExp* e) {
  ...
  if (!(t1.match(inttype) || t1.match(longtype)) || !(t2.match(inttype) || t2.match(longtype))) {
    cout << "Tipos en BinExp deben de ser int o long" << endl;
    exit(0);
  }
  ImpType result;
  switch(e->op) {
